...
def verify_format_py(file_path: Path) -> bool:
    with file_path.open('r', encoding='utf-8') as py_file:
        try:
            ...
            print()
            print(
                f'The file named {file_path.name} was expected to be a Python script.')
            print(
                '  However, it contains characters that cannot be read as text, which means')
            print(
                '  it also may not be possible for us to run the program.  If, for example,')
            print(
                '  this is an image or a document you wrote in a word processor, but that')
            print(
                '  you renamed so its filename ends in .py, you\'ll need to instead ensure')
...
def can_create(submission_path: Path) -> bool:
    if submission_path.exists():
        ...
        if what_to_do.startswith('Y'):
            print(
                f'  A new version of {submission_path.name} will be created,')
...
def create_submission(submission_path: Path, script_dir_path: Path) -> bool:
    files_to_submit = [
        file for file in script_dir_path.iterdir() if is_submittable(file)]
    files_to_skip = [file for file in script_dir_path.iterdir()
    ...
    with zipfile.ZipFile(submission_path, mode='w') as submission_zip:
        ...
        for file in files_to_skip:
            print(
                f'  Skipping {file.name}, because it should not be submitted')
